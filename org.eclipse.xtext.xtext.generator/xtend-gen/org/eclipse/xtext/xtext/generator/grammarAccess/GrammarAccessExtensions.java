...
public class GrammarAccessExtensions {
  ...
  private String process(final String input, final String prefix) {
    ...
    final String commonWhitespace = this.commonLeadingWhitespace(((List<String>)Conversions.doWrapArray(_converted_lines_1)));
    for (int n = 0; (n < lines.length); n++) {
      String _substring = (lines[n]).substring(commonWhitespace.length());
      String _plus = (prefix + _substring);
      lines[n] = _plus;
    }
    final String[] _converted_lines_2 = (String[])lines;
    return IterableExtensions.join(((Iterable<?>)Conversions.doWrapArray(_converted_lines_2)), System.lineSeparator());
  }
  ...
  private String commonLeadingWhitespace(final List<String> lines) {
    ...
    String current = Strings.repeat(" ", lines.get(0).length());
